<?php
/**
 * synjson.php - Memorial Plaques Synagogue/Society JSON API
 *
 * Cloned from cemjson.php (JOWBR cemetery endpoint). Differences:
 *   1. Queries the `memorial` table instead of `cemetery`
 *      (same column layout: cemeteryid, cem_name, landsman, country,
 *       city, state, street, burials, photos, comments, date_live...)
 *   2. Top-level JSON key is 'synagogue' instead of 'cemetery'.
 *      All inner field names are identical to cemjson.php so the
 *      cemetery.php frontend pattern ports mechanically.
 *      NOTE: in the memorial table, `burials` = number of memorials.
 *
 * Usage:
 *   /databases/synjson.php?id=AUS-00813
 *
 * CHW + JG40
 */
...
if (!$row) {
    ...
    echo json_encode(array('error' => 'Synagogue/Society not found'));
    exit;
}
